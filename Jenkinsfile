                    ...
                    default:
                        withDockerRegistry([credentialsId: "${docker_registry_credId}", url: "https://${docker_img_apps}/"]) {
                            ...
                            stage("Build app image"){
                                ...
                                pushToNexus(
                                        ...
                                        "${docker_img_apps}/bosa-cities-assets:${job_base_name}-${build_number}"
                                )
                                // This will push the app image to registry
                                pushToNexus(
                                        "${docker_registry_credId}",
                                        "https://${docker_int_app}/",
                                        "${docker_int_app}/bosa-cities:${job_base_name}-${build_number}"
                                )
                            }
                        }
                        stage('Deploy app to dev'){
                            kubeDeploy(
                                    ...
                                    ["${docker_img_apps}/bosa-cities:$job_base_name-$build_number", "${docker_img_apps}/bosa-cities-assets:$job_base_name-$build_number"]
                            )
                        }
                        stage('Deploy sidekiq to dev'){
                            kubeDeploy(
                                    ...
                                    "bosa-dev",
                                    ["bosa-cities-sidekiq" ],
                                    ["${docker_img_apps}/bosa-cities:$job_base_name-$build_number"]
                            )
                        }
                        break
                    ...
                    case ~/^rc-\d+\.\d+\.\d+$/:
                        withDockerRegistry([credentialsId: "${docker_registry_credId}", url: "https://${docker_img_apps}/"]) {
                            ...
                            stage("Build app image"){
                                ...
                                pushToNexus(
                                        "${docker_registry_credId}",
                                        "https://${docker_img_apps}/",
                                        "${docker_img_apps}/bosa-cities-assets:$job_base_name"
                                )
                                // This will push the app image to registry
                                pushToNexus(
                                        "${docker_registry_credId}",
                                        "https://${docker_img_apps}/",
                                        "${docker_img_apps}/bosa-cities:$job_base_name"
                                )
                            }
                        }
                        stage('Deploy app to uat'){
                            kubeDeploy(
                                    ...
                                    "bosa-uat",
                                    ["bosa-cities", "bosa-cities-assets" ],
                                    ["${docker_img_apps}/bosa-cities:$job_base_name", "${docker_img_apps}/bosa-cities-assets:$job_base_name"]
                            )
                        }
